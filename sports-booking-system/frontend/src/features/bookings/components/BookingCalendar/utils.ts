...
export function slotKey(slot: { date: string; startTime: string; endTime?: string; courtSurfaceId?: string | null; courtSubId?: string | null }) {
  const sub = slot.courtSurfaceId || slot.courtSubId || "";
  return slot.endTime
    ? `${slot.date}|${sub}|${slot.startTime}|${slot.endTime}`
    : `${slot.date}|${sub}|${slot.startTime}`;
}
