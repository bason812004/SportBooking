...
export function RegisterPartnerPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl rounded-md border border-line bg-white p-6 shadow-sm">
      <h1 className="text-2xl font-semibold">Đăng ký đối tác</h1>
      {step === 1 ? (
        <form className="mt-5 grid gap-4 md:grid-cols-2" onSubmit={form.handleSubmit((values) => requestMutation.mutate(values))}>
          <Input label="Họ tên" {...form.register("fullName")} error={form.formState.errors.fullName?.message ? t(form.formState.errors.fullName.message) : undefined} />
          <Input label="Email" type="email" {...form.register("email")} error={form.formState.errors.email?.message ? t(form.formState.errors.email.message) : undefined} />
          <Input label="Số điện thoại" {...form.register("phone")} />
          <Input label="Mật khẩu" type="password" {...form.register("password")} error={form.formState.errors.password?.message ? t(form.formState.errors.password.message) : undefined} />
          <Input label="Tên đơn vị" {...form.register("businessName")} error={form.formState.errors.businessName?.message ? t(form.formState.errors.businessName.message) : undefined} />
          <Input label="Địa chỉ kinh doanh" {...form.register("address")} error={form.formState.errors.address?.message ? t(form.formState.errors.address.message) : undefined} />
          <Button className="md:col-span-2" disabled={requestMutation.isPending}>
