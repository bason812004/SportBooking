  ...
  return (
    <div className="overflow-auto rounded-md border border-line bg-white">
      <table className="w-full min-w-[760px] text-sm">
        <thead className="bg-field text-left"><tr><th className="p-3">Ho ten</th><th>Email</th><th>Role</th><th>Status</th><th></th></tr></thead>
        <tbody>
          {users.data?.items.map((user) => (
            <tr key={user.id} className="border-t border-line">
              <td className="p-3 font-medium">{user.fullName}</td><td>{user.email}</td><td>{user.role}</td><td>{user.status}</td>
              <td className="p-3 text-right"><Button variant="secondary" onClick={() => toggle.mutate({ id: user.id, status: user.status })}>{user.status === "ACTIVE" ? "Khoa" : "Mo khoa"}</Button></td>
            </tr>
          ))}
        </tbody>
      </table>
