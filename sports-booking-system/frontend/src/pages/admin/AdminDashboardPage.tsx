...
export function AdminDashboardPage() {
  ...
  const stats = [
    { label: "Tổng User", value: dashboardValue(raw, ["users", "totalUsers", "userCount"], 24592).toLocaleString("vi-VN"), note: `+12% ${t("tháng này")}`, icon: Users, tone: "bg-[#dcfce7]" },
    { label: "Tổng Partner", value: dashboardValue(raw, ["partners", "totalPartners", "partnerCount"], 1204).toLocaleString("vi-VN"), note: `+5% ${t("tháng này")}`, icon: UserRoundCheck, tone: "bg-blue-100" },
    { label: "Doanh thu hệ thống", value: `${dashboardValue(raw, ["revenue", "totalRevenue"], 845000000).toLocaleString("vi-VN")} đ`, note: `+8.2% ${t("tháng này")}`, icon: WalletCards, tone: "bg-red-100" },
    { label: "Sân chờ duyệt", value: dashboardValue(raw, ["pendingCourts", "courtsPending"], 18).toLocaleString("vi-VN"), note: t("Cần xử lý ngay"), icon: FolderCheck, tone: "bg-[#eaf3e7]" }
  ];
  ...
  return (
    <div>
      <div className="flex flex-wrap items-start justify-between gap-6">
        <div>
          <h1 className="text-6xl font-black tracking-tight">{t("Tổng quan hệ thống")}</h1>
          <p className="mt-5 text-2xl text-slate-700">{t("Chào mừng Admin trở lại. Đây là tình hình hôm nay.")}</p>
        </div>
        ...
        {stats.map((item, index) => (
          <div key={item.label} className={`rounded-3xl border border-[#dfe8dc] bg-white p-8 shadow-sm ${index === 3 ? "border-l-4 border-l-red-700" : ""}`}>
            <div className="flex justify-between gap-4">
              <p className="text-xl font-bold tracking-wide">{t(item.label)}</p>
      ...
      <div className="mt-14 grid gap-8 xl:grid-cols-[1fr_360px]">
        <section className="rounded-3xl border border-[#dfe8dc] bg-white p-10 shadow-sm">
          <div className="flex items-center justify-between">
            <h2 className="text-4xl font-black">{t("Tăng trưởng tổng quan")}</h2>
            <button className="rounded-xl border border-[#b9cdb7] bg-[#f1fbef] px-6 py-3">{t("30 ngày gần nhất")}</button>
          </div>
        ...
        <section className="rounded-3xl border border-[#dfe8dc] bg-white p-10 shadow-sm">
          <div className="flex items-start justify-between">
            <h2 className="text-4xl font-black leading-tight">{t("Yêu cầu chờ duyệt")}</h2>
            <a href="/admin/courts/pending" className="text-xl font-bold text-blue-700">{t("Xem tất cả")}</a>
          </div>
          <div className="mt-8 space-y-5">
            {["Sân bóng mini Lê Văn Sỹ", "CLB Tennis Bình Thạnh", "Cụm cầu lông Gò Vấp"].map((name, index) => (
              <div key={name} className="rounded-2xl border border-[#b9cdb7] bg-[#f1fbef] p-5">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-slate-600">{index === 1 ? t("Sân tennis") : t("Sân bóng đá")}</p>
                    <p className="text-slate-600">• {t("Quận")} {index + 3}</p>
                  </div>
                  <button className="rounded-full bg-[#02712a] px-4 py-2 font-bold text-white">{t("Duyệt")}</button>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="mt-14 flex flex-wrap items-center gap-6 rounded-3xl border border-[#dfe8dc] bg-white p-8 shadow-sm">
        ...
        <div className="flex-1">
          <h2 className="text-2xl font-bold">{t("Đồng bộ blockchain")}</h2>
          <p className="text-lg text-slate-700">{t("Tất cả sổ cái đặt sân đã được bảo mật và đồng bộ.")}</p>
        </div>
        <p className="font-bold">{t("Vừa xong")} (Block #1428593)</p>
        <p className="border-l border-[#b9cdb7] pl-8">{t("Trạng thái node")} <span className="ml-3 text-[#00b83e]">● {t("Khỏe")}</span></p>
      </section>
    </div>
  );
}
