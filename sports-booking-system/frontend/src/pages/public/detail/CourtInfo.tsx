...
export function CourtInfo({ name, category, address, openingHours, rating, reviewCount, bookingCount }: { name: string; category: string; address: string; openingHours: string; rating: number | string; reviewCount: number; bookingCount: number }) {
  return (
    <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-4 flex items-center gap-2 text-slate-600"><MapPin className="h-5 w-5 text-[#0f766e]" /> {address} • Cách bạn 2.4 km</p>
