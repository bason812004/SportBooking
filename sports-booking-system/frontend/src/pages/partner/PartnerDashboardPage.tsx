...
export function PartnerDashboardPage() {
  ...
  const stats = [
    { label: "Tong so san", value: data.courts, note: "San sang hoat dong", icon: UserRoundCheck, tone: "bg-[#dcfce7]" },
    { label: "Booking hom nay", value: data.bookings, note: "+12% so voi hom qua", icon: CalendarDays, tone: "bg-blue-100" },
    { label: "Doanh thu thang", value: `${Number(data.revenue).toLocaleString("vi-VN")}d`, note: "+5.2% so voi thang truoc", icon: WalletCards, tone: "bg-red-100" },
    { label: "Yeu cau cho duyet", value: 7, note: "Can xu ly ngay", icon: CalendarDays, tone: "bg-[#24c866] text-[#063310]" }
  ];

  return (
    <div>
      <h1 className="text-6xl font-black tracking-tight">Tong quan</h1>
      <p className="mt-5 text-2xl text-slate-700">Chao mung tro lai, xem hieu suat cua san hom nay.</p>
        ...
        {stats.map((item) => (
          <div key={item.label} className={`rounded-3xl border border-[#dfe8dc] p-8 shadow-sm ${item.tone.includes("text") ? item.tone : "bg-white"}`}>
            <div className="flex justify-between gap-4">
              <p className="text-2xl leading-snug">{item.label}</p>
      ...
      <div className="mt-16 grid gap-8 lg:grid-cols-[1fr_360px]">
        <section className="rounded-3xl border border-[#dfe8dc] bg-white p-12 shadow-sm">
          <div className="flex items-center justify-between">
            <h2 className="text-4xl font-black">Xu huong dat san (Tuan)</h2>
            <button className="rounded-xl bg-[#f1fbef] px-6 py-3">Tuan nay</button>
          </div>
        ...
        <section className="rounded-3xl border border-[#dfe8dc] bg-white p-10 shadow-sm">
          <h2 className="text-4xl font-black leading-tight">Trang thai san nhanh</h2>
          <div className="mt-10 space-y-6">
            {["San Tennis A", "San Cau Long 1", "San Bong Da Mini"].map((name, index) => (
              <div key={name} className="flex items-center gap-5 rounded-2xl bg-[#f1fbef] p-5">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-slate-700">{index === 0 ? "Dang co khach (Con 45p)" : index === 1 ? "Trong" : "Sap den gio (14:00)"}</p>
                </div>
      ...
      <section className="mt-16 rounded-3xl border border-[#dfe8dc] bg-white p-12 shadow-sm">
        <div className="flex items-center justify-between">
          <h2 className="text-4xl font-black">Booking moi nhat</h2>
          <a className="font-bold text-blue-700" href="/partner/bookings">Xem tat ca</a>
        </div>
      </section>
    </div>
  );
