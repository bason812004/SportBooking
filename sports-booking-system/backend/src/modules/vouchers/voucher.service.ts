...
export const voucherService = {
  ...
  async claim(userId: string, voucherId: string) {
    ...
    if (voucher.usageLimit != null && voucher.usedCount >= voucher.usageLimit) throw new ValidationError("Voucher da het luot su dung");
    if (await voucherRepository.userVoucher(userId, voucherId)) throw new ConflictError("Ban da nhan voucher nay", "VOUCHER_ALREADY_CLAIMED");
  ...
  async updatePartner(userId: string, id: string, input: any) {
    ...
    if (!existing) throw new NotFoundError("Khong tim thay voucher cua ban");
    if (input.courtId && !(await voucherRepository.partnerCourt(input.courtId, profile.id))) {
  ...
  async deletePartner(userId: string, id: string) {
    const profile = await voucherRepository.partnerProfile(userId);
    if (!profile) throw new ForbiddenError("Tai khoan doi tac chua co ho so");
    if (!(await voucherRepository.findPartnerVoucher(id, profile.id))) throw new NotFoundError("Khong tim thay voucher cua ban");
