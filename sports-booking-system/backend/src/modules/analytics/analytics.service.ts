...
export async function trackEvent(input: Parameters<typeof analyticsRepository.trackEvent>[0]) {
  try {
...
export const analyticsService = {
  async partnerOverview(userId: string) {
  ...
  async partnerRevenue(userId: string) {
    const partnerId = await partnerIdForUser(userId);
    return analyticsRepository.partnerRevenue(partnerId);
  },

  async partnerPeakHours(userId: string) {
    const partnerId = await partnerIdForUser(userId);
  ...
  async partnerOccupancy(userId: string) {
  ...
  async partnerVouchers(userId: string) {
    const partnerId = await partnerIdForUser(userId);
  ...
  async partnerTournaments(userId: string) {
    const partnerId = await partnerIdForUser(userId);
