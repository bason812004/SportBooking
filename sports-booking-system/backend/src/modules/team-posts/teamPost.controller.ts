...
export const teamPostController = {
  ...
  createMessage: asyncHandler(async (req, res) =>
    sendSuccess(res, await teamPostService.createMessage(req.params.id, req.user!.id, req.body), 201)
  ),

  reactMessage: asyncHandler(async (req, res) =>
    sendSuccess(res, await teamPostService.reactToMessage(req.params.id, req.user!.id, req.body.messageId, req.body.reaction))
  ),

  removeReaction: asyncHandler(async (req, res) =>
    sendSuccess(res, await teamPostService.removeReaction(req.params.id, req.user!.id, req.params.messageId))
  ),

  leaveGroup: asyncHandler(async (req, res) =>
    sendSuccess(res, await teamPostService.leaveGroup(req.params.id, req.user!.id))
  ),

  removeMember: asyncHandler(async (req, res) =>
    sendSuccess(res, await teamPostService.removeMember(req.params.id, req.user!.id, req.params.memberId))
  ),

  transferAdmin: asyncHandler(async (req, res) =>
    sendSuccess(res, await teamPostService.transferAdmin(req.params.id, req.user!.id, req.body.newAdminUserId))
  )
};
